	    ...
	    int n_TPC_layers =40)
{
  ...
  if(do_svtx){
    gROOT->LoadMacro("G4_Svtx_pileup.C"); 
	    ...
	    const float      magfield_rescale=1.0)
{
  ...
  if(decayType != TPythia6Decayer::kAll) g4Reco->set_force_decay(decayType);
  
  double        fieldstrength;
  istringstream stringline(field);
  stringline>>fieldstrength;
  if(stringline.fail()){ // conversion to double fails -> we have a string
    if(field.find("sPHENIX.root")!=string::npos) g4Reco->set_field_map(field,1);
    else                                         g4Reco->set_field_map(field,2);
  }
  else{
  ...
  // if(do_cemc) radius=CEmc_Vis(g4Reco,radius,8,absorberactive);
  // for visualization substructure of SPACAL, slow to render  
  //----------------------------------------
  // HCALIN
  ...
  blackhole->set_double_param("length",g4Reco->GetWorldSizeZ()-no_overlapp); 
