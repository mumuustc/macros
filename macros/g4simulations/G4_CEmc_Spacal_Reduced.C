...
CEmc_1DProjectiveSpacal(PHG4Reco* g4Reco, double radius, const int crossings, const int absorberactive = 0)
{
  ...
  double cemc_eta_coverage = 0.6;
  double cemc_z_length = emc_outer_radius / tan( 2 * atan( exp (-cemc_eta_coverage) ) );

  assert(cemc_z_length)>0;
...
//  boundary check
  ...
  cemc ->SetLengthViaRapidityCoverage(false);
  cemc ->get_geom().set_zmin( - cemc_z_length );
  cemc ->get_geom().set_zmax( + cemc_z_length );
