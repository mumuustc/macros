...
//=======================================
double TPCDriftVelocity =3./1000.; // cm/ns
double TPCShapingRMSLead=32.; // ns, rising RMS equivalent of shaping amplifier for 80 ns SAMPA
double TPCShapingRMSTail=48.; // ns, falling RMS equivalent of shaping amplifier for 80 ns SAMPA
double TPCADCClock      =53.; // ns, corresponds to an ADC clock rate of 18.8 MHz
...
double Svtx(PHG4Reco *g4Reco,double radius,const int absorberactive=0,int verbosity=0)
{
  ...
  if(n_maps_layer>0){
    ...
    // Layer 0: radius 23.44 mm to sensor center, tilt from normal to radial vector:
    //          17.37 degrees (0.303 rad), spacing btw sensor centers: 30 deg, arc spacing 12.27 mm
    // Layer 1: radius 31.54 mm to sensor center, ttilt from normal to radial vector:
    //          17.53 degrees (0.306 rad), spacing btw sensor centers: 22.5 deg, arc spacing 12.38 mm
    // Layer 2: radius 39.29 to sensor center, tilt from normal to radial vector:
    //          17.02 degrees (0.297 rad), spacing btw sensor centers: 18.0 deg, arc spacing 12.34 mm
    ...
    double phi_tilt[3]       ={0.304,0.304,0.304};
    // radians, from the gdml file, 0.304 radians is 17.4 degrees
    for(int ilayer=0;ilayer<n_maps_layer;ilayer++){
      if(verbosity) cout << "Create Maps layer " << ilayer
      ...
      lyr->set_int_param("N_staves",staves_in_layer[ilayer]);
      // uses fixed number of staves regardless of radius, if set.
      // Otherwise, calculates optimum number of staves
      // The cell size is used only during pixilization of sensor hits,
      // but it is convemient to set it now because the geometry object needs it
  ...
  if(n_intt_layer>0){
    ...
    for(int i=0;i<n_intt_layer;i++){
      vpair.push_back(std::make_pair(n_maps_layer+i,i));
      // sphxlayer=n_maps_layer+i corresponding to inttlayer=i
      if(verbosity) cout << "Create strip tracker layer " << vpair[i].second
			 << " as  sphenix layer  " << vpair[i].first << endl;
    }
  ...
  // Kapton X_0 = 28.6 cm  // mocks up Kapton + carbon fiber structure
  ...
  // leave this change until later - will require some parameter retuning
  ...
  for(int ilayer=n_maps_layer+n_intt_layer;ilayer<(n_maps_layer+n_intt_layer+npoints);++ilayer){
    if(verbosity) cout << "Create TPC gas layer " << ilayer
		       << " with radius " << radius  << " cm "
		       << " thickness " << delta_radius - 0.01 
		       << " length " << cage_length << endl;
...
void Svtx_Cells(int verbosity=0)
{
  ...
  if(n_intt_layer>0){
    ...
    reco->set_timing_window_defaults(-40.,60.);
  ...
  if(do_tpc_distortion){
    ...
    string TPC_distortion_file=string(getenv("CALIBRATIONROOT"))+
      Form("/Tracking/TPC/SpaceChargeDistortion/TPCCAGE_20_78_211_2.root"); 
    PHG4TPCSpaceChargeDistortion *tpc_distortion=new PHG4TPCSpaceChargeDistortion(TPC_distortion_file);
  ...
  if(n_gas_layer==40){
    ...
    // r-phi: diffusion + GEM smearing = 750 microns, assume resolution is 
    //        20% of that => 150 microns
    // Z    : amplifier shaping time (RMS 32 ns, 48 ns) and drift vel of 
    //        3 cm/microsec gives smearing of 3 x (32+48/2 = 1.2 mm, 
    //        assume resolution is 20% of that => 240 microns   
    svtx_cells->setSmearRPhi(0.10); 
    // additional random displacement of cloud positions wrt hits to give 
    // expected cluster resolution of 150 microns for charge at membrane 
    svtx_cells->setSmearZ(0.15);
    // additional random displacement of cloud positions wrt hits to give 
    // expected cluster rsolution of 240 microns for charge at membrane
  }
  else{
    // 60 layer tune
  ...
  for(int i=n_maps_layer+n_intt_layer;i<Max_si_layer;++i){
    ...
    svtx_cells->set_timing_window(i,-17500.0,+17500.0);
  }
...
void Svtx_Reco(int verbosity=0)
{
  ...
  for(int i=0;i<n_maps_layer;++i){
    digi->set_adc_scale(i,255,0.4e-6);
    // reduced by a factor of 2.5 when going from maps thickess of 50 microns to 18 microns
  }  
  if(n_intt_layer>0){
    // INTT
  ...
  for(int i=0;i<n_maps_layer;i++){  
    ...
    deadarea->set_hit_efficiency(i,1.);
  }
  ...
  // maps 
  for(int i=0;i<n_maps_layer;i++){  
    // reduced by x2.5 when going from cylinder maps with 50 microns thickness to actual maps
    // with 18 microns thickness
    // Note the non-use of set_using_thickness here, this is so that the shortest dimension of
    // the cell sets the mip energy loss
  ...
  for(int i=n_maps_layer;i<n_maps_layer+n_intt_layer;i++){
    clusterizer->set_z_clustering(i,false); // default
    // if(i==3) clusterizer->set_z_clustering(i,true);
    // else     clusterizer->set_z_clustering(i,false);
  }
  ...
  if(n_gas_layer==40){
    ...
    tpcclusterizer->setFitWindowSigmas(0.0160,0.0160);
    // should be changed when TPC cluster resolution changes
  ...
  kalman->Verbosity(0);
  if(use_primary_vertex) kalman->set_fit_primary_tracks(true);
...
void Svtx_Eval(std::string outputfile,int verbosity=0)
{
  ...
  // SVTX evaluation
  //----------------
  //  SvtxEvalHist *eval=new SvtxEvalHist("SVTXEVALHIST",outputfile.c_str());
